...
TLORMEngine.Entities.OnPlatform.prototype.init = function(args) {
	...
	this.addComponent(new TLORMEngine.Components.Render2D({ fill_colour: args.fill_colour, z: args.z }));
	this.addComponent(new TLORMEngine.Components.Gravity({ g: args.g || 2, terminal_velocity: args.terminal_velocity || 15 }));
	...
	data_component.setData("falling", true);
	...
	if (!game.condition_manager.getCondition("is_falling")) {
		game.condition_manager.addCondition(new TLORMEngine.Conditions.Condition({
			name: "is_falling",
			type: "Data", function: "getData", function_args: ["falling"], check: "=", value: true,
		}));
	}
	if (!game.condition_manager.getCondition("not_falling")) {
		game.condition_manager.addCondition(new TLORMEngine.Conditions.Condition({
			name: "not_falling",
			type: "Data", function: "getData", function_args: ["falling"], check: "=", value: false,
		}));
	}
